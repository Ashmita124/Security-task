...
import { FaArrowLeft, FaArrowRight, FaChevronDown, FaChevronRight, FaClipboardList, FaCog, FaHome, FaListAlt, FaPlusCircle, FaTags, FaUser } from 'react-icons/fa';
...
const menuItems = [
    ...
    {
        title: 'Items',
        icon: FaListAlt, // Changed from FaUtensils to FaListAlt for a more generic 'items' look
        subMenu: [
            { title: 'All Items', icon: FaListAlt, path: '/admin/items/all-items' }, // path updated from /menu/ to /items/
            { title: 'Add Item', icon: FaPlusCircle, path: '/admin/items/add-item' }
        ]
    },
    {
        title: 'Categories',
        ...
        <div className={cn('fixed h-screen bg-gray-900 text-white flex flex-col', { 'w-64': !collapsed, 'w-20': collapsed })}>
            <div className="flex items-center justify-between p-4">
                ...
                <Button
                    ...
                    className="transition-transform ml-auto text-white hover:bg-gray-700 hover:text-[#00bf63]"
                >
                    ...
                    <div key={index}>
                        <Button
                            ...
                            className={cn('w-full flex items-center justify-between p-3', {
                                'text-[#00bf63] bg-gray-800': selectedItem === index,
                                'text-white bg-transparent': selectedItem !== index,
                                'hover:text-[#00bf63] hover:bg-gray-700': true,
                            })}
                            onClick={() => handleItemClick(index, item.path)}
                        >
                            <div className="flex items-center">
                                <item.icon className={cn('text-xl mr-3', { 'text-[#00bf63]': selectedItem === index })} />
                                ...
                                {item.subMenu.map((subItem, subIndex) => (
                                    <Button
                                        ...
                                        className={cn('w-full flex items-center justify-start p-2', {
                                            'text-[#00bf63] bg-gray-800': selectedSubMenu === subIndex,
                                            'text-white bg-transparent': selectedSubMenu !== subIndex,
                                            'hover:text-[#00bf63] hover:bg-gray-700': true,
                                        })}
                                        onClick={() => handleSubMenuClick(index, subIndex, subItem.path)}
                                    >
                                        <subItem.icon className={cn('text-xl mr-3', { 'text-[#00bf63]': selectedSubMenu === subIndex })} />
