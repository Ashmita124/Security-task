...
function MyOrders() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:3000/api/v1/customers/${customerId}`, {
        headers: {
  ...
  return (
    <>
      <Navbar />
      <div className="min-h-screen bg-[#F3F4F6] text-black font-sans px-6 md:px-10 py-10 flex flex-col md:flex-row gap-10">
          ...
          <div className="grid gap-6">
            {orders.map((order) => (
              ...
              >
                ...
                <p className="text-lg font-bold text-gray-800 mb-4">Total: Rs. {order.totalAmount.toFixed(2)}</p>
                {order.deliveryDate && (
