  ...
  try {
    ...
    if (!recaptchaResponse.data.success) {
      await ActivityLog.create({
        ...
        success: false,
        details: `reCAPTCHA verification failed: ${JSON.stringify(recaptchaResponse.data)}`,
        ipAddress: req.ip || req.connection.remoteAddress,
      }).catch((logError) => console.error('ActivityLog Error:', logError.message));
      return res.status(400).json({
        success: false,
        message: `reCAPTCHA verification failed: ${recaptchaResponse.data['error-codes']?.join(', ') || 'Unknown error'}`,
      });
    }
  } catch (error) {
